...
export function ReportTicketTable({ title, rows, empty, returnTo = "/reports/weekly" }: { title: string; rows: ReportTicketRow[]; empty: string; returnTo?: string }) {
  return (
    <section className="rounded-[18px] border border-white/[0.08] bg-white/[0.035] p-3 shadow-sm shadow-black/20">
      ...
      <div className="space-y-2 md:hidden">
        {rows.length ? rows.map((row) => <TicketCard key={row.id} row={row} returnTo={returnTo} />) : <Empty text={empty} />}
      </div>

...
export default async function WeeklyReportPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return (
    <div className="page-shell mx-auto max-w-7xl space-y-4 pb-28 md:pb-8">
      ...
      <ReportTicketTable title="Виконані заявки" rows={data.tickets.completed} empty="Виконаних заявок за період немає." />
      <ReportTicketTable title="Невиконані заявки" rows={data.tickets.unresolved} empty="Невиконаних заявок за період немає." />
      <ReportTicketTable title="Очікують підтвердження" rows={data.tickets.waitingConfirmation} empty="Заявок на підтвердженні немає." />

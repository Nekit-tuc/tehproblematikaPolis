...
export default async function WorkPlanDetailPage({ params, searchParams }: PageProps) {
  ...
  return (
    <div className="page-shell space-y-6 pb-32 md:pb-8">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <Button asChild variant="outline" size="sm" className="mb-3 min-h-10 rounded-2xl md:rounded-md">
            <Link href="/work-planning"><ArrowLeft className="h-4 w-4" />До планування</Link>
          </Button>
          <h1 className="break-words text-2xl font-semibold">{plan.title}</h1>
        ...
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
          ...
          {isDraft ? (
            <form action={sendWorkPlanAction.bind(null, plan.id)}>
              <SubmitButton className="min-h-11 w-full rounded-2xl md:min-h-0 md:w-auto md:rounded-md" pendingText="Надсилається...">
      ...
      <CardContent className="space-y-4">
        <form action={updateWorkPlanAction.bind(null, plan.id)} className="grid gap-3 md:grid-cols-4">
          ...
          <div className="flex items-end">
            <SubmitButton className="min-h-11 w-full rounded-2xl md:min-h-0 md:rounded-md" pendingText="Зберігається...">Зберегти</SubmitButton>
          </div>
        ...
        <form action={cancelWorkPlanAction.bind(null, plan.id)}>
          <SubmitButton variant="destructive" className="min-h-11 w-full rounded-2xl md:w-auto md:rounded-md" pendingText="Скасовується...">
            Скасувати план
          </SubmitButton>
        </form>
      </CardContent>
      ...
      <div className="grid gap-2 md:flex md:justify-end">
        ...
        {plan.status === "draft" && ticket ? (
          <form action={removeWorkPlanItemAction.bind(null, plan.id)}>
            ...
            <SubmitButton variant="outline" size="sm" className="min-h-10 w-full rounded-2xl md:min-h-0 md:w-auto md:rounded-md" pendingText="...">
              Прибрати
            </SubmitButton>
          </form>
        ) : null}

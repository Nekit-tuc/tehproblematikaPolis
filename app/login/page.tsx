...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const message = error ? errorMessages[error] ?? "Не вдалося увійти." : null;
    ...
    <main className="grid min-h-screen place-items-center bg-[radial-gradient(circle_at_top_left,rgba(209,134,59,.2),transparent_32%),#171717] px-4">
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          <CardTitle className="text-2xl">Polissya Service Desk</CardTitle>
          <CardDescription>Вхід для співробітників технічної служби та керівників об'єктів.</CardDescription>
        </CardHeader>
